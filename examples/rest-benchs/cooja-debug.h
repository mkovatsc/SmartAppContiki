...
#define COOJA_DEBUG_INT(val) do { char tmp[10] = {0}; uint32_t v = (val); int i=9; if(v==0) tmp[--i] = '0'; while(v) { tmp[--i] = '0' + v%10; v /= 10; } cooja_debug_ptr = tmp+i;  } while(0)
#define COOJA_DEBUG_INTH(val) do { char tmp[5] = {0}; uint16_t v = (val); tmp[0] = HEXC(v>>12); tmp[1] = HEXC(v>>8); tmp[2] = HEXC(v>>4); tmp[3] = HEXC(v); cooja_debug_ptr = tmp;  } while(0)
